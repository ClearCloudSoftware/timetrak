...
function HourScale() {
  return (
    <div className="relative border-r border-separator bg-surface-alt text-[10px] text-label-2">
      {Array.from({ length: 24 }, (_, h) => (
        <div key={h} className="relative h-10 border-b border-separator px-1.5 pt-0.5">
...
function TimelinePane({
  ...
  return (
    <div className="relative h-full">
      <div className="relative h-full">
        {Array.from({ length: 24 }, (_, h) => (
          <div key={h} className="absolute left-0 right-0 h-px bg-fill-hover" style={{ top: h * 40 }} />
        ))}
        {entries.map((e) => {
          ...
          return (
            <button
              key={e.id}
              onClick={() => onEdit(e)}
              className="absolute left-2 right-3 overflow-hidden rounded-md text-left text-[11px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] ring-1 ring-inset ring-separator hover:brightness-95"
              style={{
                top,
                height,
            ...
            >
              <div className="px-2 py-0.5">
                <div className="truncate font-medium text-label">
                  {cat?.name}{proj ? ` · ${proj.name}` : ''}
                </div>
                <div className="text-[10px] tabular-nums text-label/70">
